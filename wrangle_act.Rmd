...
* name variable has some entries starting with low case letters(example: An).
* expanded_urls contains link which are not validm possible because they are expired

*`prediction`* table
* variable img_num is not needed  

* where it is needed change variable types to the appropriate 
...
Let rating_denominator be 10 for all entries. Hence, because all the entries have only one value 10, then drop this varible   
...
If rating_numerator is higer than 15 let it be 15 
...
```{python}
type(prediction.p1[0])
```

```{python}
type(prediction.p2[0])
```

```{python}
type(prediction.p3[0])
```
